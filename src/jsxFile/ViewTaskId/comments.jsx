...
import OneCmt from './OneCmt'
import NewQuick from '../Help/NewQuick'
export default function comments({ act, setAct }) {
    const [my, setMsg] = useState({ message: '', msg: "", loading: false });

    function changeinp(e) {
        let value = e.target.value;
        if (value.length > 150) value = value.slice(0, 150);
        setMsg((pre) => ({ ...pre, message: value }));
    }
    async function sndCmt() {
        try {
            if (my.message == "") return;
            const search = localStorage.getItem("type").endsWith('u') ? "public":"personal";
            setMsg((pre) => ({ ...pre, loading: true, }));
            const response = await fetch(`/api/addCmt/${search}`, {
                method: "POST",
                headers: {
            ...
            if (!data.error) {
                setAct((pre) => ({ ...pre, extra: { ...pre.extra, cmt: data.data } }))
                setMsg((pre) => ({ ...pre, loading: false, message: '', msg: data.message }));
            } else {
                setMsg((pre) => ({ ...pre, loading: false, msg: data.message }));
            }
        } catch (err) {
            console.log("Error from server:", err);
            setMsg((pre) => ({ ...pre, loading: false, msg: err.message || "Server side error " }));
        }

    }

    return (<div className='commentsview isFlex'>
        <div className="inpview">
            ...
            {my.message && <NewQuick msg={my.msg} setMsg={setMsg} />}
        </div>
        <div className="allcmts isFlex">
            {act?.extra.cmt?.map((one, ind) => (
                <OneCmt key={ind} setAct={setAct} my={my} setMsg={setMsg} user={act.userId} userId={act.task.owner._id} cmt={one} taskId={act.task._id} />
            ))}
        </div>
    </div>)
}