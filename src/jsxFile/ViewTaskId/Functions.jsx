...
export default function functions({ user, taskId, isDone, setAct, title }) {
    ...
    async function toggletask(mth) {
        ...
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/markToggle/${taskId}/${search}`, {
                method: "POST",
                headers: {
        ...
        async function killTask() {
            ...
            try {
                ...
                const search = localStorage.getItem("type").endsWith('u') ? "public" : "personal";
                const response = await fetch(`${import.meta.env.VITE_API_URL}/api/killTask/${taskId}`, {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json",
                        "Authorization": `Bearer ${localStorage.getItem("token")}`,
